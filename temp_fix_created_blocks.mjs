...
console.log('=== FETCHING ON-CHAIN BLOCK DATA FOR CREATED EVENTS ===');

// Cache block timestamps to avoid duplicate RPC calls
const blockCache = {};

async function getBlockTimestamp(blockHex) {
  if (blockCache[blockHex]) return blockCache[blockHex];
  const block = await rpc('eth_getBlockByNumber', [blockHex, false]);
  const ts = block ? parseInt(block.timestamp, 16) : null;
  blockCache[blockHex] = ts;
  return ts;
}
...
for (const hashId of NEW_HASHIDS) {
  ...
  if (!tx) {
    console.log(`#${tokenId}: TX NOT FOUND ON CHAIN`);
    continue;
  }

  const blockNumber = parseInt(tx.blockNumber, 16);
  const tsUnix = await getBlockTimestamp(tx.blockNumber);
  const blockTimestamp = new Date(tsUnix * 1000).toISOString();

  console.log(`#${tokenId}: block=${blockNumber} ts=${blockTimestamp}`);

  const from = tx.from?.toLowerCase();

  // Update the created event in DB: fix blockNumber, blockTimestamp, from, to
  const { error, count } = await sb.from('events')
    .update({ blockNumber, blockTimestamp, from, to: from }, { count: 'exact' })
    .eq('hashId', hashId)
    .eq('type', 'created');
...
console.log(`\nTotal updated: ${updatedCount} / ${NEW_HASHIDS.length}`);

// Also update ethscriptions.createdAt to match
console.log('\n=== UPDATING ethscriptions.createdAt ===');
for (const hashId of NEW_HASHIDS) {
  const tokenId = TOKEN_MAP[hashId];
  const tx = await rpc('eth_getTransactionByHash', [hashId]);
  if (!tx) continue;
  const tsUnix = await getBlockTimestamp(tx.blockNumber);
  const createdAt = new Date(tsUnix * 1000).toISOString();
  const { error } = await sb.from('ethscriptions').update({ createdAt }).eq('hashId', hashId);
  if (error) {
    console.log(`#${tokenId} ethscriptions UPDATE ERROR: ${error.message}`);
  } else {
    console.log(`#${tokenId}: createdAt=${createdAt} ✅`);
  }
}
