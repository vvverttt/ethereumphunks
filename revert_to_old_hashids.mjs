...
// Revert Supabase: swap new → old
for (const { tokenId, oldHashId, newHashId } of swaps) {
  const { error } = await sb.from('ethscriptions')
    .update({ hashId: oldHashId, oldHashId: null })
    .eq('hashId', newHashId);
  if (error) console.error(`#${tokenId} Supabase error: ${error.message}`);
  else console.log(`#${tokenId} Supabase reverted ✓`);
}
...
for (const { tokenId, oldHashId, newHashId } of swaps) {
  ...
  if (item.id?.toLowerCase() === newHashId.toLowerCase()) {
    item.id = oldHashId;
    console.log(`#${tokenId} JSON reverted ✓`);
  } else {
    console.log(`#${tokenId} JSON already has: ${item.id?.slice(0,20)}...`);
  }
}
