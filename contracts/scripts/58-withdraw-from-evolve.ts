...
// 15 Quantum DystoPhunks
const DYSTO_HASHIDS: string[] = [
...
async function main() {
  ...
  const allHashIds = [...MISSING_HASHIDS, ...DYSTO_HASHIDS];
  console.log(`\nWithdrawing ${allHashIds.length} tokens (8 missing + 15 dysto)...\n`);
  ...
  for (const hashId of allHashIds) {
    try {
      ...
      console.error(`❌ ${hashId.slice(0, 16)}... error: ${e.message?.slice(0, 80)}`);
      failed++;
    }
  }
