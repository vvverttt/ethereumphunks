...
async function main() {
  ...
  console.log('Checking depositor for OLD hashIds:\n');
  let inContract = 0;
  ...
  for (const item of OLD_HASHIDS) {
    ...
    if (dep !== ZERO) {
      inContract++;
      console.log(`#${item.tokenId}: STILL DEPOSITED (depositor: ${dep})`);
    } else {
      console.log(`#${item.tokenId}: cleared`);
    }
  }
  console.log(`\n${inContract}/23 old hashIds still deposited`);
}
