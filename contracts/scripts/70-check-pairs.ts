...
// The 23 quantum hashIds with their tokenIds
const QUANTUM_TOKENS: Record<string, number> = {
...
async function main() {
  ...
  console.log(`Total pairs registered on-chain: ${pairCount}`);
  console.log('');

  let allGood = true;
  for (const [quantumHash, tokenId] of Object.entries(QUANTUM_TOKENS)) {
    // Get pairId for this quantum hash
    const pairId = await contract.pairIdOf(quantumHash as `0x${string}`);
    const isRegistered = await contract.registered(quantumHash as `0x${string}`);
    const isOg = await contract.isOg(quantumHash as `0x${string}`);
    const dep = await contract.depositor(quantumHash as `0x${string}`);

    // Get the OG hashId this pairs with
    const ogHash = await contract.ogHashId(pairId);

    // Look up what cryptophunk the OG hashId is
    const { data: ogPhunk } = await sb.from('ethscriptions')
      .select('tokenId, slug')
      .eq('hashId', ogHash)
      .single();

    const deposited = dep !== '0x0000000000000000000000000000000000000000';
    const pairsCorrectToken = ogPhunk?.tokenId === tokenId;
    const ok = isRegistered && !isOg && deposited && pairsCorrectToken;
    if (!ok) allGood = false;

    console.log(
      `#${tokenId}: pairId=${pairId} registered=${isRegistered} isOg=${isOg} deposited=${deposited} ` +
      `ogHash=${ogHash.slice(0,18)}... → OG tokenId=${ogPhunk?.tokenId ?? 'NOT FOUND'} (${ogPhunk?.slug ?? '?'}) ${ok ? '✅' : '❌'}`
    );
  }

  console.log(allGood ? '\nAll 23 pairs correct ✅' : '\nIssues found ❌');
}
