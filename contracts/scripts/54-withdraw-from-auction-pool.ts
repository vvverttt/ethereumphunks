...
async function main() {
  ...
  const poolSizeBefore = await contract.poolSize();
  console.log('Pool size before:', poolSizeBefore.toString());

  const gas = await contract.withdrawFromPoolBatch.estimateGas(POOL_HASHIDS);
  const feeData = await hre.ethers.provider.getFeeData();
  console.log('Estimated gas:', gas.toLocaleString(), '=', hre.ethers.formatEther(gas * feeData.gasPrice!), 'ETH\n');

  const tx = await contract.withdrawFromPoolBatch(POOL_HASHIDS);
  console.log('Tx:', tx.hash);
  const receipt = await tx.wait();
  console.log('✅ Confirmed block', receipt?.blockNumber);

  const poolSizeAfter = await contract.poolSize();
  console.log('Pool size after:', poolSizeAfter.toString());
}
