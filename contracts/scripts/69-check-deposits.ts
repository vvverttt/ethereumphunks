...
import { PhilipLotteryV68 } from '../typechain-types';
...
async function main() {
  const contract = await hre.ethers.getContractAt('PhilipLotteryV68', PROXY_ADDRESS) as unknown as PhilipLotteryV68;

  try {
    const poolSize = await contract.poolSize();
    console.log(`Pool size on-chain: ${poolSize}`);
  } catch { console.log('poolSize() unavailable'); }
  console.log('');

  let allGood = true;
  for (const [hashId, tokenId] of Object.entries(TOKEN_MAP)) {
    const inPool = await contract.inPool(hashId as `0x${string}`);
    const depositor = await contract.depositor(hashId as `0x${string}`);
    const ok = inPool && depositor !== '0x0000000000000000000000000000000000000000';
    if (!ok) allGood = false;
    console.log(`#${tokenId}: inPool=${inPool} depositor=${depositor.slice(0,16)}... ${ok ? '✅' : '❌'}`);
  }

  console.log(allGood ? '\nAll 23 registered in contract ✅' : '\nSome not registered ❌');
}
