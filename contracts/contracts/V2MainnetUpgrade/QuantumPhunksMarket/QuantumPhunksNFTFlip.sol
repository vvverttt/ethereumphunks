...
contract CryptoPhunksV67Flip is
    ICreatorToken,
    ERC721Upgradeable,
    ERC2981Upgradeable,
    OwnableUpgradeable,
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        // Every token gets an interactive animation_url: the SAME stored image, mirrored on
        // click. Phunks face left; this shows the right side without storing a second copy.
        // Animated tokens keep animating while flipped — a CSS transform mirrors the element,
        // it does not touch the decoder, so the GIF/APNG plays exactly as before.
        // `animated` is no longer needed here: the page below serves both cases.
        animated;
        string memory anim = string(abi.encodePacked(',"animation_url":"', _flipPage(img), '"'));
    ...
    /// @dev The click-to-flip page served as `animation_url`. Built from the stored image at
    ///      read time, so it costs NOTHING per token — no extra storage, no extra write.
    ///      The static `image` stays the left-facing SVG, so left is what shows first in every
    ///      grid, thumbnail and wallet that does not render animation_url.
    function _flipPage(string memory dataUri) internal view returns (string memory) {
        string memory bg = bytes(backgroundColor).length > 0 ? backgroundColor : "000000";
        bytes memory html = abi.encodePacked(
            ...
            'image-rendering:pixelated;cursor:pointer;transition:transform .25s}',
            '.r{transform:scaleX(-1)}</style><img id=p src="', dataUri,
            '"><script>p.onclick=function(){p.classList.toggle("r")}</script>'
        );
