...
console.log('=== CREATED EVENT CHECK ===');
console.log('Format: #tokenId | hashId (new) | txHash in created event | match?');
console.log('');
...
for (const hashId of NEW_HASHIDS) {
  ...
  const { data: evts } = await sb.from('events')
    .select('type, blockNumber, blockTimestamp, txHash, "from", "to"')
    .eq('hashId', hashId)
    .eq('type', 'created');

  if (!evts?.length) {
    console.log(`#${tokenId}: NO CREATED EVENT ❌`);
    wrongCount++;
    continue;
  }

  const e = evts[0];
  const txHashMatchesHashId = e.txHash === hashId;

  if (!txHashMatchesHashId) {
    console.log(`#${tokenId}: txHash WRONG ❌`);
    console.log(`  hashId:  ${hashId}`);
    console.log(`  txHash:  ${e.txHash}`);
    console.log(`  block: ${e.blockNumber}, from: ${e.from}, to: ${e.to}`);
    wrongCount++;
  } else {
    console.log(`#${tokenId}: txHash OK ✅ (block ${e.blockNumber})`);
  }
}

console.log(`\nWrong: ${wrongCount} / ${NEW_HASHIDS.length}`);
