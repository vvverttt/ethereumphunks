...
/**
 * LOCAL-ONLY background preview tool for the "header + grid + dropdown" scheme, scoped to
 * the two Quantum collections (quantummissingphunksv67 + quantumdystophunkzv67) ONLY.
 *
 ...
 * live `--base-color` (the header/grid/dropdown background) and picks a readable
 * `--highlight` text color by luminance, so gold backgrounds automatically switch to
 * dark text. It also stamps `[data-bgpick]` on :root so the collections-dropdown CSS
 * hook tracks the previewed color (button + the active row).
 *
 * The preview only applies while you're on one of the TARGET_SLUGS collections — on
 * cryptophunksv67 (or anything else) it does nothing, so v67 keeps its real blue theme.
 * "Which collection am I on" is read from :root[data-collection] (set by ThemeService for
 * each collection's market slug). The choice persists in localStorage and re-applies on
 * every NavigationEnd, so switching between qMissing and qDysto keeps the gold on both.
 *
 * This is a design aid, not a user feature. To turn a chosen color into the real default,
 * update ThemeService.collectionOverrides[<slug>].
 */
@Component({
  standalone: true,
    ...
    @if (enabled) {
      <div class="bgpick" [class.collapsed]="!open()">
        <button type="button" class="bgpick-toggle" (click)="open.set(!open())">
          <span>BG COLOR — qMissing + qDysto</span>
...
export class BgColorPickerComponent {
  ...
  private readonly STORE_KEY = 'qmq_bg_override';

  /** The preview only ever applies on these two collections — never v67 or anything else. */
  private readonly TARGET_SLUGS = ['quantummissingphunksv67', 'quantumdystophunkzv67'];
  ...
  readonly groups: { label: string; items: Swatch[] }[] = [
    {
      ...
      items: [
        ...
        { hex: '#648595', name: 'Blue-Grey' },
        { hex: '#79fae6', name: 'Aqua' },
        { hex: '#66cdff', name: 'Sky Blue', tag: 'current' },
  ...
  constructor(private router: Router) {
    ...
    if (saved) this.active.set(saved.toLowerCase());

    // ThemeService re-writes --base-color + data-collection on each collection change, so
    // re-check after navigation settles (60ms): paint only when on a target collection,
    // otherwise clear the flag so v67 / other collections keep their own theme untouched.
    this.router.events.pipe(
      filter((e): e is NavigationEnd => e instanceof NavigationEnd),
    ).subscribe(() => setTimeout(() => this.applyOrClear(), 60));

    // Catch the very first load (its NavigationEnd may fire before this component exists).
    setTimeout(() => this.applyOrClear(), 120);
  }
  ...
  reset(): void {
    ...
    document.documentElement.removeAttribute('data-bgpick');
    location.reload();
  }

  /** True when the collection currently in view is one of the two Quantum targets. */
  private onTarget(): boolean {
    return this.TARGET_SLUGS.includes(document.documentElement.dataset['collection'] || '');
  }

  /** Paint the preview if a color is chosen AND we're on a target collection; otherwise
   *  drop the [data-bgpick] flag so ThemeService's real theme (v67 blue, default lime) wins. */
  private applyOrClear(): void {
    if (this.active() && this.onTarget()) this.paint(this.active());
    else document.documentElement.removeAttribute('data-bgpick');
  }

  /** Write --base-color (as an "r, g, b" triplet) and a readable text color to :root,
   *  and flag [data-bgpick] so the collections-dropdown CSS hook tracks the preview. */
  private paint(hex: string): void {
    ...
    // Inert in production — this attribute is only ever set by this localhost tool.
    root.setAttribute('data-bgpick', '');
  }
