...
export class ItemViewComponent {
  ...
  singlePhunk$ = this.route.params.pipe(
    filter((params: any) => !!params.hashId || (!!params.contract && !!params.tokenId)),
    distinctUntilChanged((prev, curr) =>
      prev.hashId === curr.hashId && prev.contract === curr.contract && prev.tokenId === curr.tokenId),
    switchMap((params: any) => params.hashId
      ? this.dataSvc.fetchSinglePhunk(params.hashId)
      : this.dataSvc.fetchSinglePhunkByTokenId(slugFromContract(params.contract) || '', params.tokenId)),
    tap((phunk: any) => {
      if (phunk?.slug) {
  ...
  async transferPhunk(phunk: Phunk, address?: string): Promise<void> {
    ...
    try {
      ...
      if (set) {
        // ERC-721C (e.g. cryptophunksv67): move via transferFrom on the NFT contract.
        // The owner is exempt from the operator whitelist, so a direct transfer works
        // even though third-party marketplace approvals stay blocked.
        hash = await this.web3Svc.transferNft(set.nft, this.posTokenId(phunk), toAddress);
      } else {
        await this.checkConsenus(phunk);
        hash = await this.web3Svc.transferPhunk(hashId, toAddress);
      }
      notification = {
        ...notification,
        type: 'pending',
