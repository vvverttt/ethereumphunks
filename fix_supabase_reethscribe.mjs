...
for (const { tokenId, oldHashId, newHashId } of swaps) {
  ...
  // Check what tables reference this hashId
  const { data: events } = await sb.from('events').select('id').eq('hashId', oldHashId);
  console.log(`  events referencing old hashId: ${events?.length ?? 0}`);

  // Delete old events (they're for invalid ethscriptions, not useful)
  if (events?.length) {
    ...
    console.log(`  deleted ${events.length} old events`);
  }

  // Now update ethscriptions hashId
  const { error: ethErr } = await sb.from('ethscriptions')
    .update({ hashId: newHashId, oldHashId: oldHashId })
    .eq('hashId', oldHashId);
  if (ethErr) console.error(`  update ethscriptions error: ${ethErr.message}`);
  else console.log(`  hashId updated: ${oldHashId.slice(0,20)}... → ${newHashId.slice(0,20)}...`);
}
