...
for (const [hashId, tokenId] of Object.entries(TOKEN_MAP)) {
  const [{ data: eth }, { data: evts }] = await Promise.all([
    sb.from('ethscriptions').select('tokenId, slug, owner, sha').eq('hashId', hashId).single(),
    sb.from('events').select('type, blockNumber, blockTimestamp').eq('hashId', hashId).order('blockNumber'),
  ]);

  const hasCreated = evts?.some(e => e.type === 'created');
  const hasDeposit = evts?.some(e => e.blockNumber === 24655712);
  const createdTs = evts?.find(e => e.type === 'created')?.blockTimestamp;
  const correctDate = createdTs && new Date(createdTs) >= new Date('2026-01-01');
  const ok = eth && hasCreated && hasDeposit && evts?.length === 2 && correctDate;

  if (!ok) allGood = false;
  console.log(
    `#${tokenId}: ${eth ? eth.slug : 'NO ETHSCRIPTION'} | ` +
    `events=${evts?.length ?? 0} | created=${hasCreated ? createdTs?.slice(0,10) : 'MISSING'} | deposit=${hasDeposit} ${ok ? '✅' : '❌'}`
  );
}
console.log(allGood ? '\nAll 23 fully registered ✅' : '\nSome issues ❌');
